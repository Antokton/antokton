  ...
  return (
    <>
      <motion.div
        ...
        {post.image_url && (
          <button type="button" onClick={() => setImageOpen(true)} className="relative block h-48 w-full overflow-hidden text-left" aria-label="Hap foton e plotë">
      ...
      <PhotoLightbox
        ...
        title={post.title || "Foto"}
        postText={post.description || ""}
        authorName={post.author_name || "Anonim"}
        authorHref={post.author_url || ""}
        notificationKey={post.id || post.slug || post.title || ""}
      />
    </>
  );
}
