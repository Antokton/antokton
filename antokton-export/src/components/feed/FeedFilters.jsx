...
function ProfessionAutocomplete({ value, onChange }) {
  ...
  return (
    <div className={`relative ${focused ? "z-[9999]" : "z-10"}`}>
      <Input
        type="text"
        value={currentValue}
      ...
      {focused && suggestions.length > 0 && (
        ...
        >
          <button
            ...
            onMouseDown={(event) => event.preventDefault()}
            onClick={() => chooseProfession("all")}
            className="block w-full px-3 py-1.5 text-left text-xs font-medium leading-4 text-white hover:!bg-[#1f2937] focus:!bg-[#1f2937]"
          >
          ...
          {suggestions.map((profession) => (
            <button
              ...
              className="block w-full px-3 py-1.5 text-left text-xs font-normal leading-4 text-white hover:!bg-[#1f2937] focus:!bg-[#1f2937]"
            >
              {profession}
            </button>
          ))}
